...
function TeamTab() {
  ...
  const generateLink = async () => {
    ...
    try {
      ...
      toast.error(e?.message || "تعذّر توليد الرابط");
  ...
  return (
    <div className="space-y-5">
              ...
              {pending.map((iv) => (
                <div key={iv.id} className="rounded-2xl border border-dashed border-border p-3 flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {iv.token && (
                      <Button
                        size="icon" variant="ghost" title="نسخ الرابط"
                        className="h-9 w-9 rounded-full text-muted-foreground hover:text-primary"
                        onClick={() => copy(`${window.location.origin}/join/${iv.token}`)}
                      >
      ...
      {/* دعوة عضو */}
      <AlertDialog open={inviteOpen} onOpenChange={(v) => { if (!v) { setInviteOpen(false); setLink(""); } }}>
        <AlertDialogContent dir="rtl">
          <AlertDialogHeader>
          ...
          <div className="space-y-3 text-right">
            <div className="space-y-1.5">
              ...
              <Select value={inviteRole} onValueChange={(v) => { setInviteRole(v as AppRole); setLink(""); }}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent dir="rtl">
                  {ALL_ROLES.map((r) => (
                    <SelectItem key={r} value={r}>{ROLE_LABEL[r]}</SelectItem>
            ...
            <div className="rounded-2xl bg-muted/50 p-3 ring-1 ring-border/60">
              <div className="flex items-center justify-between gap-2">
                <span className="text-xs text-muted-foreground">رابط دعوة بصلاحية «{ROLE_LABEL[inviteRole]}»</span>
                <Button size="sm" variant="outline" className="rounded-full gap-1.5" onClick={generateLink} disabled={linking}>
                  <Link2 className="w-3.5 h-3.5" />
                  {linking ? "جاري التوليد…" : "ولّد رابط"}
                </Button>
              </div>
              {link && (
                <div className="mt-2 flex items-center gap-2">
                  <Input dir="ltr" readOnly value={link} className="text-[11px]" onFocus={(e) => e.currentTarget.select()} />
                  <Button size="icon" variant="ghost" className="h-9 w-9 shrink-0 rounded-full" onClick={() => copy(link)}>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={sending}>إغلاق</AlertDialogCancel>
